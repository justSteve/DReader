...
- **Interactive programs** that expect a real terminal (htop, vim, claude itself, gc
  start when it attaches to sessions, any REPL)
- **Long-running services or daemons** that must persist past the Claude Code
  session (background servers, watchers, feeds)
